...
public class H1 extends AbstractHtml {
    ...
    private static final Logger LOGGER = Logger.getLogger(H1.class.getName());

    private static final PreIndexedTagName PRE_INDEXED_TAG_NAME = PreIndexedTagName.H1;
    ...
    private static void warnForUnsupportedAttributes(final AbstractAttribute... attributes) {
        for (final AbstractAttribute abstractAttribute : attributes) {
            if (!(abstractAttribute != null && (abstractAttribute instanceof H1Attributable
                    || abstractAttribute instanceof GlobalAttributable))) {
                LOGGER.warning(abstractAttribute + " is not an instance of H1Attribute");
            }
        }
    }
