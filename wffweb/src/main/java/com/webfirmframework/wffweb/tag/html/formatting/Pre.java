...
public class Pre extends AbstractHtml {
    ...
    private static final Logger LOGGER = Logger.getLogger(Pre.class.getName());

    private static final PreIndexedTagName PRE_INDEXED_TAG_NAME = PreIndexedTagName.PRE;
    ...
    private static void warnForUnsupportedAttributes(final AbstractAttribute... attributes) {
        for (final AbstractAttribute abstractAttribute : attributes) {
            if (!(abstractAttribute != null && (abstractAttribute instanceof PreAttributable
                    || abstractAttribute instanceof GlobalAttributable))) {
                LOGGER.warning(abstractAttribute + " is not an instance of PreAttribute");
            }
        }
    }
