 ...
 * @since 2.1.8
 */
class WffDataUpdateListenerImpl implements WffDataUpdateListener {

    private static final long serialVersionUID = 1L;

    public static final Logger LOGGER = Logger
            .getLogger(WffDataUpdateListenerImpl.class.getName());

    private BrowserPage browserPage;

    @SuppressWarnings("unused")
    private WffDataUpdateListenerImpl() {
        throw new AssertionError();
    }

    WffDataUpdateListenerImpl(final BrowserPage browserPage) {
    ...
    public void updatedWffData(final UpdateEvent event) {
        ...
        try {
            ...
            final WffData wffData = event.getWffData();
            ...
            nameValue.setValues(new byte[][] { dataWffIdBytes,
                    event.getKey().getBytes("UTF-8"), wffData.build(true) });
