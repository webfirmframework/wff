...
public class WffJsFileTest {
    ...
    private static String expectedContent = """
            ...
            * v2;var v3 = v18
            ...
            throwInvalidSetURIArgException();}
            var uriBefore = cURI();if(uri === uriBefore || uri === window.location.href){
            return;}
            if(replace){
            window.history.replaceState({}, document.title, uri);}else{
            window.history.pushState({}, document.title, uri);}
            var uriAfter = cURI();if(uriBefore !== uriAfter){
            ...
            'values': [[wffGlobal.uriEventInitiator.BROWSER], encoder.encode(l.pathname + l.search)]
            };v76.push(pthNV);}
