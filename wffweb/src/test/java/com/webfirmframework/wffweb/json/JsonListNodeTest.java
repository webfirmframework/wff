...
public class JsonListNodeTest {
    ...
    public void testSizeAwareConvertTo() {
        ...
        class CustomJsonList extends JsonList {
            public CustomJsonList(final int initialCapacity) {
                super(initialCapacity);
            }
        }
        ...
        final CustomJsonList convertedObject1 = jsonList1.sizeAwareConvertTo(
                CustomJsonMap::new, CustomJsonMap::put,
                CustomJsonList::new, CustomJsonList::add, true);
    ...
    @Test
    public void testToWffBMArray() {
        JsonList jsonList = new JsonList();
        jsonList.add(new JsonValue("one", JsonValueType.STRING));
        jsonList.add(new JsonValue("two", JsonValueType.STRING));
        jsonList.add(new JsonValue("three", JsonValueType.STRING));

        WffBMArray wffBMArray = jsonList.toWffBMArray(true);
        Assert.assertEquals("one", wffBMArray.getValueAsString(0));
        Assert.assertEquals("two", wffBMArray.getValueAsString(1));
        Assert.assertEquals("three", wffBMArray.getValueAsString(2));
    }
